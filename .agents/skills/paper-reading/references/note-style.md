...
- Keep main equations as rendered LaTeX when the surface supports it.
- In variable explanations, use readable symbols or plain names rather than raw LaTeX source. For example, write `θ¹`, `πθ`, `Dtext`, `Iₜ`, and `Ĩₜ` instead of `$\\theta^{1}$`, `$\\pi_\\theta$`, `$D_{text}$`, `$I_t$`, or `$\\tilde{I}_t$`.
- Explain each important variable's meaning and role in Chinese, then add a short intuition for the equation.
- Provide copyable LaTeX source only when the user explicitly asks for it.
