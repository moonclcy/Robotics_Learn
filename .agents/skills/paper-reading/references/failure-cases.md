...
## Unreadable Table Or Screenshot

Symptom: a response includes a PDF page render or screenshot, but table labels, numbers, or dense plot annotations are too small to read in chat.

Cause: the output satisfied "image present" but failed the user-facing readability check, usually because a whole-page render was used for a small-font table or wide result figure.

Do differently:

- Use Markdown tables as the default for result tables, ablations, and benchmark comparisons.
- If a visual is still useful, crop tightly around the table or plot and render at higher DPI.
- Preview the cropped image before sending and verify labels and numbers are readable.
- Do not use whole-page screenshots as the primary evidence for small-font material.
